...
public class HospitalDemo {
    public static void main(String[] args) {
        ...
        //Bez koristenje na forEach funkcijata
        //Consumer<Doctor> doctorConsumer = d -> System.out.println(d.getName() + "(" + d.getLicenseNumber() + ")" + d.getLevel() + " " + d.getPatients());

        //So koristenje na forEach funkcijata
        er.forEach(d -> System.out.println(d));

        //TODO: Print all doctors that treat using Consumer + forEach ---
        System.out.println("\n=== All Doctors ===");
        Consumer<Doctor> print = System.out::println;
        er.forEach(print);
        ...
        Predicate<Doctor> morePatients = d -> d.getPatients() > 20;
        Predicate<Doctor> higherExpertise = d -> d.getLevel() >= 7;
        Predicate<Doctor> full = morePatients.and(higherExpertise);

        Doctor[] passing = er.filter(full);
        ...
        //er.forEach(doctor -> doctor.setLevel(doctor.getLevel() + 1));
        er.mutate(doctor -> doctor.setLevel(doctor.getLevel() + 1));
        er.forEach(doctor -> System.out.println(doctor));
        ...
        er.conditionalMutate(doctor -> doctor.getPatients() > 30, doctor -> doctor.setLevel(doctor.getLevel() + 1));

        //TODO: Map doctors to labels in the format: Name: name, Level: level
        System.out.println("\n=== Map doctors to labels ===");
        String[] mappers = er.mapToLabels(doctor -> "Name: " + doctor.getName() + ", Level: " + doctor.getLevel());
