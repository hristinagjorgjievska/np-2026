...
class ComplexNumber<T extends Number, U extends Number> implements Comparable<ComplexNumber<?,?>>{
    ...
    public double modul(){
        double result = Math.pow(real.doubleValue(), 2) + Math.pow(imaginary.doubleValue(), 2);
    ...
    public String toString() {
        double realNum = real.doubleValue();
        double imaginaryNum = imaginary.doubleValue();

        if (imaginaryNum >= 0){
            return String.format("%.2f+%.2fi", realNum, imaginaryNum);
        }
        else {
            return String.format("%.2f-%.2fi", realNum, Math.abs(imaginaryNum));
        }
    }
}
